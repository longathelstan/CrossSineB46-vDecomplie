...
implements TypeBasedGuardingDynamicLinker {
    ...
    private static final boolean rangeCheck(Object array, Object index2) {
        double doubleValue;
        if (!(index2 instanceof Number)) {
            return false;
        }
        Number n = (Number)index2;
        int intIndex = n.intValue();
        if ((double)intIndex != (doubleValue = n.doubleValue()) && !Double.isInfinite(doubleValue)) {
            return false;
        }
    ...
    private static final boolean rangeCheck(List<?> list, Object index2) {
        double doubleValue;
        if (!(index2 instanceof Number)) {
            return false;
        }
