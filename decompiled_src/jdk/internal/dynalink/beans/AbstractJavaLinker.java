...
implements GuardingDynamicLinker {
    ...
    private static SingleDynamicMethod createDynamicMethod(AccessibleObject m2) {
        MethodHandle mh;
        if (CallerSensitiveDetector.isCallerSensitive(m2)) {
            return new CallerSensitiveDynamicMethod(m2);
        }
        try {
            mh = AbstractJavaLinker.unreflectSafely(m2);
        }
        catch (IllegalAccessError e) {
            return new CallerSensitiveDynamicMethod(m2);
        }
        Member member = (Member)((Object)m2);
        return new SimpleDynamicMethod(mh, member.getDeclaringClass(), member.getName(), m2 instanceof Constructor);
    }
