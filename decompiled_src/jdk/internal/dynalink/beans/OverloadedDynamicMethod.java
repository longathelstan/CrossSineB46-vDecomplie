...
extends DynamicMethod {
    ...
    public MethodHandle getInvocation(CallSiteDescriptor callSiteDescriptor, LinkerServices linkerServices) {
        ...
        while (it.hasNext()) {
            SingleDynamicMethod m2 = (SingleDynamicMethod)it.next();
            if (OverloadedDynamicMethod.isApplicableDynamically(linkerServices, callSiteType, m2)) continue;
            it.remove();
        }
    ...
    public boolean contains(SingleDynamicMethod m2) {
        for (SingleDynamicMethod method : this.methods) {
            if (!method.contains(m2)) continue;
            return true;
        }
        return false;
    }
    ...
    public String toString() {
        ...
        int len = 0;
        for (SingleDynamicMethod m2 : this.methods) {
