...
implements IntElements {
    ...
    private Object[] toObjectArray(boolean trim) {
        ...
        for (int index2 = 0; index2 < len; ++index2) {
            oarray[index2] = this.array[index2];
        }
        return oarray;
    }
    ...
    private double[] toDoubleArray() {
        ...
        for (int index2 = 0; index2 < len; ++index2) {
            darray[index2] = this.array[index2];
        }
        return darray;
    }
    ...
    public ArrayData set(int index2, Object value, boolean strict) {
        ...
        if (value == ScriptRuntime.UNDEFINED) {
            return new UndefinedArrayFilter(this).set(index2, value, strict);
        }
        ArrayData newData = this.convert(value == null ? Object.class : value.getClass());
        return newData.set(index2, value, strict);
    }
    ...
    public ArrayData set(int index2, double value, boolean strict) {
        if (JSType.isRepresentableAsInt(value)) {
            this.array[index2] = (int)value;
            this.setLength(Math.max((long)(index2 + 1), this.length()));
            return this;
        }
        return this.convert(Double.class).set(index2, value, strict);
    }
