...
extends ArrayData {
    ...
    public final boolean hasRoomFor(int index2) {
        return this.has(index2) || (long)index2 == this.length() && this.ensure(index2) == this;
    }
    ...
    protected final int throwHas(int index2) {
        if (!this.has(index2)) {
    ...
    public GuardedInvocation findFastGetIndexMethod(Class<? extends ArrayData> clazz, CallSiteDescriptor desc, LinkRequest request) {
        ...
        if (ContinuousArrayData.class.isAssignableFrom(clazz) && indexType == Integer.TYPE && this.has(index2 = ((Integer)(args2 = request.getArguments())[args2.length - 1]).intValue())) {
    ...
    public GuardedInvocation findFastSetIndexMethod(Class<? extends ArrayData> clazz, CallSiteDescriptor desc, LinkRequest request) {
        ...
        if (ContinuousArrayData.class.isAssignableFrom(clazz) && indexType == Integer.TYPE && this.hasRoomFor(index2 = ((Integer)(args2 = request.getArguments())[args2.length - 2]).intValue()) && (setElement = this.getElementSetter((Class<?>)elementType)) != null) {
            MethodHandle getArray = ScriptObject.GET_ARRAY.methodHandle();
