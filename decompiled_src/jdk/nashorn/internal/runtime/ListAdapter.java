...
Deque<Object> {
    ...
    public final Object get(int index2) {
        ...
        return this.getAt(index2);
    }

    private Object getAt(int index2) {
        return this.obj.getSlot(index2);
    }

    @Override
    public Object set(int index2, Object element) {
        this.checkRange(index2);
        Object prevValue = this.getAt(index2);
        this.obj.setSlot(index2, element);
        return prevValue;
    }

    private void checkRange(int index2) {
        if (index2 < 0 || index2 >= this.size()) {
            throw ListAdapter.invalidIndex(index2);
        }
    }
    ...
    @Override
    public final void add(int index2, Object e) {
        block7: {
            try {
                if (index2 < 0) {
                    throw ListAdapter.invalidIndex(index2);
                ...
                int size = this.size();
                if (index2 < size) {
                    this.getDynamicInvoker(SPLICE_ADD, SPLICE_ADD_INVOKER_CREATOR).invokeExact(this.obj.getMember("splice"), this.obj, index2, 0, e);
                    break block7;
                }
                if (index2 == size) {
                    this.addLast(e);
    ...
    private static IndexOutOfBoundsException invalidIndex(int index2) {
        return new IndexOutOfBoundsException(String.valueOf(index2));
    }
    ...
    @Override
    public final Object remove(int index2) {
        if (index2 < 0) {
            throw ListAdapter.invalidIndex(index2);
        }
        if (index2 == 0) {
        ...
        if (index2 < maxIndex) {
            Object prevValue = this.get(index2);
            this.invokeSpliceRemove(index2, 1);
            return prevValue;
        }
        if (index2 == maxIndex) {
            return this.invokePop();
