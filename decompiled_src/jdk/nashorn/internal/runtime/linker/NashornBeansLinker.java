...
implements GuardingDynamicLinker {
    ...
    private static String findFunctionalInterfaceMethodName(Class<?> clazz) {
        ...
        for (Class<?> iface : clazz.getInterfaces()) {
            ...
            for (Method m2 : iface.getMethods()) {
                if (!Modifier.isAbstract(m2.getModifiers()) || NashornBeansLinker.isOverridableObjectMethod(m2)) continue;
                return m2.getName();
            }
        }
    ...
    private static boolean isOverridableObjectMethod(Method m2) {
        switch (m2.getName()) {
            ...
            case "hashCode": {
                return m2.getReturnType() == Integer.TYPE && m2.getParameterCount() == 0;
            }
            case "toString": {
                return m2.getReturnType() == String.class && m2.getParameterCount() == 0;
            }
        }
        return false;
    }
