...
extends ScriptObject {
    ...
    private static String fixExponent(String str, boolean trimZeros) {
        int index2 = str.indexOf(101);
        if (index2 < 1) {
            return str;
        }
        int expPadding = str.charAt(index2 + 2) == '0' ? 3 : 2;
        int fractionOffset = index2;
        if (trimZeros) {
        ...
        if (fractionOffset < index2 || expPadding == 3) {
            return str.substring(0, fractionOffset) + str.substring(index2, index2 + 2) + str.substring(index2 + expPadding);
        }
        return str;
    }
