...
extends ScriptObject {
    ...
    public boolean delete(int key, boolean strict) {
        ...
        return this.isMapped(index2) ? this.deleteMapped(index2, strict) : super.delete(key, strict);
    }
    ...
    public boolean delete(Object key, boolean strict) {
        ...
        return this.isMapped(index2) ? this.deleteMapped(index2, strict) : super.delete(primitiveKey, strict);
    }
    ...
    public boolean defineOwnProperty(String key, Object propertyDesc, boolean reject) {
        ...
        if (index2 >= 0) {
            Object oldValue;
            boolean isMapped = this.isMapped(index2);
            Object object = oldValue = isMapped ? this.getArray().getObject(index2) : null;
            ...
            if (isMapped) {
                ...
                if (desc.type() == 2) {
                    this.setDeleted(index2, oldValue);
                } else if (desc.has("writable") && !desc.isWritable()) {
                    this.setDeleted(index2, desc.has("value") ? desc.getValue() : oldValue);
                } else if (desc.has("value")) {
                    this.setArray(this.getArray().set(index2, desc.getValue(), false));
                }
            }
            return true;
        }
    ...
    private boolean isDeleted(int index2) {
        return this.deleted != null && this.deleted.get(index2);
    }

    private void setDeleted(int index2, Object unmappedValue) {
        if (this.deleted == null) {
            this.deleted = new BitSet(this.numMapped);
        }
        this.deleted.set(index2, true);
        this.setUnmappedArg(index2, unmappedValue);
    }

    private boolean deleteMapped(int index2, boolean strict) {
        Object value = this.getArray().getObject(index2);
        boolean success = super.delete(index2, strict);
        if (success) {
            this.setDeleted(index2, value);
        }
        return success;
    }
    ...
    private boolean isMapped(int index2) {
        return index2 >= 0 && index2 < this.numMapped && !this.isDeleted(index2);
    }
