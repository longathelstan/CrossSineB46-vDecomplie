...
implements OptimisticBuiltins {
    ...
    NativeArray(long[] array) {
        ...
        for (int index2 = 0; index2 < array.length; ++index2) {
            long value = array[index2];
            if (widest == Integer.TYPE && JSType.isRepresentableAsInt(value)) {
                arrayData = arrayData.set(index2, (int)value, false);
                continue;
            }
            if (widest != Object.class && JSType.isRepresentableAsDouble(value)) {
                arrayData = arrayData.set(index2, value, false);
                widest = Double.TYPE;
                continue;
            }
            arrayData = arrayData.set(index2, (Object)value, false);
            widest = Object.class;
        }
    ...
    NativeArray(Object[] array) {
        ...
        ArrayData arrayData = this.getArray();
        for (int index2 = 0; index2 < array.length; ++index2) {
            Object value = array[index2];
            arrayData = value == ScriptRuntime.EMPTY ? arrayData.delete(index2) : arrayData.set(index2, value, false);
        }
        this.setArray(arrayData);
    }
    ...
    @Override
    public final void defineOwnProperty(int index2, Object value) {
    ...
    public static Object pop(Object self) {
        try {
            ...
            long index2 = len - 1L;
            Object element = sobj.get(index2);
            sobj.delete(index2, true);
            sobj.set((Object)"length", (double)index2, 2);
            return element;
        }
    ...
    public static ScriptObject sort(Object self, Object comparefn) {
        try {
            ...
            if (len > 1L) {
                ...
                while (iter.hasNext() && (index2 = iter.next().longValue()) < len) {
                    src.add(array.getObject((int)index2));
                }
