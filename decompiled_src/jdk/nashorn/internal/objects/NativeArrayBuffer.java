...
extends ScriptObject {
    ...
    static int adjustIndex(int index2, int length) {
        return index2 < 0 ? NativeArrayBuffer.clamp(index2 + length, length) : NativeArrayBuffer.clamp(index2, length);
    }

    private static int clamp(int index2, int length) {
        if (index2 < 0) {
            return 0;
        }
        if (index2 > length) {
