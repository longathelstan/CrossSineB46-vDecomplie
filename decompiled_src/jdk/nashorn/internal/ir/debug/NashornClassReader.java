...
extends ClassReader {
    ...
    private static int readByte(byte[] bytecode, int index2) {
        return (byte)(bytecode[index2] & 0xFF);
    }

    private static int readShort(byte[] bytecode, int index2) {
        return (short)((bytecode[index2] & 0xFF) << 8) | bytecode[index2 + 1] & 0xFF;
    }

    private static int readInt(byte[] bytecode, int index2) {
        return (bytecode[index2] & 0xFF) << 24 | (bytecode[index2 + 1] & 0xFF) << 16 | (bytecode[index2 + 2] & 0xFF) << 8 | bytecode[index2 + 3] & 0xFF;
    }

    private static long readLong(byte[] bytecode, int index2) {
        int hi = NashornClassReader.readInt(bytecode, index2);
        int lo = NashornClassReader.readInt(bytecode, index2 + 4);
        return (long)hi << 32 | (long)lo;
    }

    private static String readUTF(int index2, int utfLen, byte[] bytecode) {
    ...
    private void parseCode(byte[] bytecode, int index2, int len, String desc) {
        ...
        while (i < index2 + len) {
            ...
            labels.add(new NashornTextifier.NashornLabel(opcode, i - index2));
            switch (opcode & 0xFF) {
                case 196: {
                    wide = true;
                    ++i;
    ...
    extends IndexInfo {
        ...
        IndexInfo2(ArrayList<Constant> cp, int tag, int index2, int index22) {
            super(cp, tag, index2);
            this.index2 = index22;
        }
    ...
    extends Constant {
        ...
        IndexInfo(ArrayList<Constant> cp, int tag, int index2) {
            super(cp, tag);
            this.index = index2;
        }
