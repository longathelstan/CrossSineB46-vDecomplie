...
extends BaseNode {
    ...
    public IndexNode(long token, int finish, Expression base, Expression index2) {
        super(token, finish, base, false);
        this.index = index2;
    }

    private IndexNode(IndexNode indexNode, Expression base, Expression index2, boolean isFunction, Type type, int programPoint) {
    ...
    public IndexNode setIndex(Expression index2) {
        if (this.index == index2) {
            return this;
        }
        return new IndexNode(this, this.base, index2, this.isFunction(), this.type, this.programPoint);
    }
