...
public final class TokenLookup {
    ...
    public static TokenType lookupKeyword(char[] content, int position, int length) {
        ...
        if ('a' <= first && first <= 'z') {
            int index2 = first - 32;
            for (TokenType tokenType = table[index2]; tokenType != null; tokenType = tokenType.getNext()) {
    ...
    public static TokenType lookupOperator(char ch0, char ch1, char ch2, char ch3) {
        ...
        if (' ' < ch0 && ch0 <= '~' && ('a' > ch0 || ch0 > 'z')) {
            int index2 = ch0 - 32;
            block6: for (TokenType tokenType = table[index2]; tokenType != null; tokenType = tokenType.getNext()) {
                String name = tokenType.getName();
                switch (name.length()) {
                    case 1: {
                        return tokenType;
    ...
    static {
        for (TokenType tokenType : TokenType.getValues()) {
            ...
            int index2 = first - 32;
            assert (index2 < 95) : "Token name does not fit lookup table";
            int length = tokenType.getLength();
            TokenType prev = null;
            for (next = table[index2]; next != null && next.getLength() > length; next = next.getNext()) {
                prev = next;
            }
            tokenType.setNext(next);
            if (prev == null) {
                TokenLookup.table[index2] = tokenType;
                continue;
            }
