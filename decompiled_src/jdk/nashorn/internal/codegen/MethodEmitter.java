...
public class MethodEmitter {
    ...
    MethodEmitter load(Symbol symbol, Type type) {
        ...
        if (symbol.hasSlot()) {
            ...
            int index2 = symbol.getFieldIndex();
            if (this.functionNode.needsArguments()) {
                this.debug((Object)"load symbol", (Object)symbol.getName(), (Object)" arguments index=", (Object)index2);
                this.loadCompilerConstant(CompilerConstants.ARGUMENTS);
                this.load(index2);
                ScriptObject.GET_ARGUMENT.invoke(this);
            } else {
                this.debug((Object)"load symbol", (Object)symbol.getName(), (Object)" array index=", (Object)index2);
    ...
    void arraystore() {
        ...
        Type index2 = this.popType(Type.INT);
        assert (index2.isInteger()) : "array index is not integer, but " + index2;
    ...
    void store(Symbol symbol, Type type, boolean onlySymbolLiveValue) {
        ...
        if (symbol.hasSlot()) {
            ...
            if (this.functionNode.needsArguments()) {
                ...
                this.debug((Object)"store symbol", (Object)symbol.getName(), (Object)" arguments index=", (Object)index2);
                this.loadCompilerConstant(CompilerConstants.ARGUMENTS);
                this.load(index2);
                ArgumentSetter.SET_ARGUMENT.invoke(this);
            } else {
                this.convert(Type.OBJECT);
                this.debug((Object)"store symbol", (Object)symbol.getName(), (Object)" array index=", (Object)index2);
                this.loadCompilerConstant(CompilerConstants.VARARGS);
                this.load(index2);
    ...
    MethodEmitter dynamicGetIndex(Type result, int flags, boolean isMethod) {
        ...
        if ((index2 = this.peekType()).isObject() || index2.isBoolean()) {
            index2 = Type.OBJECT;
            this.convert(Type.OBJECT);
        }
        this.popType();
        this.popType(Type.OBJECT);
        String signature = Type.getMethodDescriptor(resultType, Type.OBJECT, index2);
    ...
    void dynamicSetIndex(int flags) {
        ...
        this.popType();
        Type index2 = this.peekType();
        if (index2.isObject() || index2.isBoolean()) {
            index2 = Type.OBJECT;
            this.convert(Type.OBJECT);
        }
        this.popType(index2);
        ...
        this.method.visitInvokeDynamicInsn("dyn:setElem|setProp", CompilerConstants.methodDescriptor(Void.TYPE, receiver.getTypeClass(), index2.getTypeClass(), value.getTypeClass()), LINKERBOOTSTRAP, flags);
    }
