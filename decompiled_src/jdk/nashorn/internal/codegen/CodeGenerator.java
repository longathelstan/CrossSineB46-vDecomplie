...
implements Loggable {
    ...
    private void storeElement(Expression[] nodes, Type elementType, int index2) {
        this.method.load(index2);
        Expression element = nodes[index2];
        if (element == null) {
    ...
    void loadConstant(String string) {
        ...
        ClassEmitter classEmitter = this.unit.getClassEmitter();
        int index2 = this.compiler.getConstantData().add(string);
        this.method.load(index2);
    ...
    private void loadConstant(Object object, CompileUnit compileUnit, MethodEmitter methodEmitter) {
        ...
        int index2 = this.compiler.getConstantData().add(object);
        Class<?> cls = object.getClass();
        if (cls == PropertyMap.class) {
            methodEmitter.load(index2);
            ...
            methodEmitter.loadConstants().load(index2).arrayload();
        ...
        private void prologue() {
            this.target.accept(new SimpleNodeVisitor(){
                ...
                public boolean enterIndexNode(IndexNode node) {
                    this.enterBaseNode();
                    Expression index2 = node.getIndex();
                    if (!index2.getType().isNumeric()) {
                        Store.this.this$0.loadExpressionAsObject(index2);
                    } else {
                        Store.this.this$0.loadExpressionUnbounded(index2);
                    }
                    Store.this.depth = Store.this.depth + index2.getType().getSlots();
                    if (Store.this.isSelfModifying()) {
