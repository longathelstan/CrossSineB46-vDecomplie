...
extends ObjectCreator<Expression> {
        ...
        for (MapTuple tuple : this.tuples) {
            ...
            if (value != null && (constantValue = LiteralNode.objectAsConstant(value)) != LiteralNode.POSTSET_MARKER) {
                ...
                if (property != null) {
                    ...
                    assert (ArrayIndex.isValidArrayIndex(index2));
                    if (longIndex >= oldLength) {
                        arrayData = arrayData.ensure(longIndex);
                    }
                    arrayData = constantValue instanceof Integer ? arrayData.set(index2, (Integer)constantValue, false) : (constantValue instanceof Double ? arrayData.set(index2, (Double)constantValue, false) : arrayData.set(index2, constantValue, false));
                    if (longIndex > oldLength) {
    ...
    public void populateRange(MethodEmitter method, Type objectType, int objectSlot, int start, int end) {
        ...
        for (int i = start; i < end; ++i) {
            ...
            if (property == null) {
                int index2 = ArrayIndex.getArrayIndex(tuple.key);
                assert (ArrayIndex.isValidArrayIndex(index2));
                method.dup();
                this.loadIndex(method, ArrayIndex.toLongIndex(index2));
