...
implements Serializable {
    ...
    public WarpGrid(int rows, int cols, int w, int h) {
        ...
        for (int row = 0; row < rows; ++row) {
            for (int col = 0; col < cols; ++col) {
                this.xGrid[index2] = (float)col * (float)(w - 1) / (float)(cols - 1);
                this.yGrid[index2] = (float)row * (float)(h - 1) / (float)(rows - 1);
                ++index2;
            }
        }
    }
    ...
    public void lerp(float t, WarpGrid destination, WarpGrid intermediate) {
        ...
        for (int row = 0; row < this.rows; ++row) {
            for (int col = 0; col < this.cols; ++col) {
                intermediate.xGrid[index2] = ImageMath.lerp(t, this.xGrid[index2], destination.xGrid[index2]);
                intermediate.yGrid[index2] = ImageMath.lerp(t, this.yGrid[index2], destination.yGrid[index2]);
                ++index2;
            }
        }
    }
    ...
    protected void interpolateSpline(float[] xKnots, float[] yKnots, int offset, int length, float[] splineY, int splineOffset, int splineLength) {
        ...
        float x0 = xKnots[index2];
        float k1 = k2 = yKnots[index2];
        float k0 = k2;
        float x1 = xKnots[index2 + 1];
        float k3 = yKnots[index2 + 1];
        for (int i = 0; i < splineLength; ++i) {
            if (index2 <= end && (float)i > xKnots[index2]) {
                k0 = k1;
                k1 = k2;
                k2 = k3;
                x0 = xKnots[index2];
                ...
                k3 = index2 < end ? yKnots[index2 + 1] : k2;
            }
    ...
    protected void interpolateSpline2(float[] xKnots, float[] yKnots, int offset, float[] splineY, int splineOffset, int splineLength) {
        int index2 = offset;
        float leftX = xKnots[index2];
        float leftY = yKnots[index2];
        float rightX = xKnots[index2 + 1];
        float rightY = yKnots[index2 + 1];
        for (int i = 0; i < splineLength; ++i) {
            if ((float)i > xKnots[index2]) {
                leftX = xKnots[index2];
                leftY = yKnots[index2];
                rightX = xKnots[++index2];
                rightY = yKnots[index2];
            }
