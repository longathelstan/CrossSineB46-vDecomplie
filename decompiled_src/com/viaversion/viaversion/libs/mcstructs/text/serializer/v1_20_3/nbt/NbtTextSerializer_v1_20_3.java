...
implements ITextComponentSerializer<Tag>,
CodecUtils_v1_20_3 {
    private final IStyleSerializer<Tag> styleSerializer;

    public NbtTextSerializer_v1_20_3(Function<NbtTextSerializer_v1_20_3, IStyleSerializer<Tag>> styleSerializer) {
    ...
    public Tag serialize(ATextComponent object) {
        ...
        if (object instanceof StringComponent) {
            ...
            if (((TranslationComponent)component).getArgs().length > 0) {
                ArrayList<Tag> args2 = new ArrayList<Tag>();
                for (Object arg : ((TranslationComponent)component).getArgs()) {
                    args2.add(this.convert(arg));
                }
                out.put("with", this.optimizeAndConvert(args2));
            }
    ...
    protected Tag optimizeAndConvert(List<Tag> tags) {
        ...
        if (commonType == null) {
            ListTag<CompoundTag> out = new ListTag<CompoundTag>();
            for (Tag tag : tags) {
        ...
        if (tag.get("text") instanceof StringTag && (type == null || type.equals("text"))) {
            ...
            if (tag.contains("with")) {
                ...
                for (i = 0; i < with.size(); ++i) {
                    Tag arg = with.get(i);
                    args2[i] = arg instanceof NumberTag ? ((NumberTag)arg).getValue() : (arg instanceof StringTag ? ((StringTag)arg).getValue() : this.deserialize(arg));
                }
