...
Serializable {
    ...
    public K remove(int index2) {
        ...
        --this.size;
        if (index2 != this.size) {
            System.arraycopy(a, index2 + 1, a, index2, this.size - index2);
        }
        a[this.size] = null;
    ...
    public boolean addAll(int index2, Collection<? extends K> c) {
        if (c instanceof ReferenceList) {
            return this.addAll(index2, (ReferenceList)c);
        }
        this.ensureIndex(index2);
        int n = c.size();
        if (n == 0) {
    ...
    @Override
    public boolean addAll(int index2, ReferenceList<? extends K> l) {
        this.ensureIndex(index2);
        int n = l.size();
        if (n == 0) {
            return false;
