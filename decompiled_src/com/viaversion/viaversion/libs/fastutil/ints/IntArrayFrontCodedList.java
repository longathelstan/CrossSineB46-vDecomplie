...
RandomAccess {
    ...
    private int length(int index2) {
        int[][] array = this.array;
        int delta = index2 % this.ratio;
        long pos = this.p[index2 / this.ratio];
    ...
    public int arrayLength(int index2) {
        this.ensureRestrictedIndex(index2);
        return this.length(index2);
    }

    private int extract(int index2, int[] a, int offset, int length) {
        long startPos;
        int delta = index2 % this.ratio;
        long pos = startPos = this.p[index2 / this.ratio];
        int arrayLength = IntArrayFrontCodedList.readInt(this.array, pos);
        int currLen = 0;
        if (delta == 0) {
            pos = this.p[index2 / this.ratio] + (long)IntArrayFrontCodedList.count(arrayLength);
    ...
    public int[] get(int index2) {
        return this.getArray(index2);
    }

    public int[] getArray(int index2) {
        this.ensureRestrictedIndex(index2);
        int length = this.length(index2);
        int[] a = new int[length];
        this.extract(index2, a, 0, length);
        return a;
    }

    public int get(int index2, int[] a, int offset, int length) {
        this.ensureRestrictedIndex(index2);
        IntArrays.ensureOffsetLength(a, offset, length);
        int arrayLength = this.extract(index2, a, offset, length);
    ...
    public int get(int index2, int[] a) {
        return this.get(index2, a, 0, a.length);
    }
