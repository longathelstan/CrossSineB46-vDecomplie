...
extends MethodVisitor {
    ...
    public void visitLocalVariable(String name, String desc, String signature, Label start, Label end, int index2) {
        this.p.visitLocalVariable(name, desc, signature, start, end, index2);
    ...
    public AnnotationVisitor visitLocalVariableAnnotation(int typeRef, TypePath typePath, Label[] start, Label[] end, int[] index2, String desc, boolean visible) {
        Printer p = this.p.visitLocalVariableAnnotation(typeRef, typePath, start, end, index2, desc, visible);
        AnnotationVisitor av = this.mv == null ? null : this.mv.visitLocalVariableAnnotation(typeRef, typePath, start, end, index2, desc, visible);
        return new TraceAnnotationVisitor(av, p);
    }
