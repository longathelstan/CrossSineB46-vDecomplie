...
class Frame {
    ...
    private static int type(ClassWriter cw, String desc) {
        int data;
        int index2 = desc.charAt(0) == '(' ? desc.indexOf(41) + 1 : 0;
        switch (desc.charAt(index2)) {
            case 'V': {
                return 0;
            }
            case 'B': 
            ...
            case 'L': {
                String t = desc.substring(index2 + 1, desc.length() - 1);
    ...
    private static boolean merge(ClassWriter cw, int t, int[] types, int index2) {
        int v;
        int u = types[index2];
        if (u == t) {
            return false;
