...
extends MethodVisitor {
    ...
    public void visitLdcInsn(Object cst) {
        ...
        if (i.type == 5 || i.type == 6) {
            this.code.put12(20, index2);
        } else if (index2 >= 256) {
            this.code.put12(19, index2);
        } else {
            this.code.put11(18, index2);
        }
    }
    ...
    public void visitLocalVariable(String name, String desc, String signature, Label start, Label end, int index2) {
        ...
        if (signature != null) {
            ...
            this.localVarType.putShort(start.position).putShort(end.position - start.position).putShort(this.cw.newUTF8(name)).putShort(this.cw.newUTF8(signature)).putShort(index2);
        }
        if (this.localVar == null) {
            this.localVar = new ByteVector();
        }
        ++this.localVarCount;
        this.localVar.putShort(start.position).putShort(end.position - start.position).putShort(this.cw.newUTF8(name)).putShort(this.cw.newUTF8(desc)).putShort(index2);
        if (this.compute != 3 && (n = index2 + ((c = desc.charAt(0)) == 'J' || c == 'D' ? 2 : 1)) > this.maxLocals) {
            this.maxLocals = n;
        }
    }
    ...
    public AnnotationVisitor visitLocalVariableAnnotation(int typeRef, TypePath typePath, Label[] start, Label[] end, int[] index2, String desc, boolean visible) {
        ...
        for (int i = 0; i < start.length; ++i) {
            bv.putShort(start[i].position).putShort(end[i].position - start[i].position).putShort(index2[i]);
        }
        if (typePath == null) {
            bv.putByte(0);
        } else {
