...
public class ClassReader {
    ...
    public String[] getInterfaces() {
        ...
        if (n > 0) {
            ...
            for (int i = 0; i < n; ++i) {
                interfaces[i] = this.readClass(index2 += 2, buf);
            }
        }
        return interfaces;
    }
    ...
    private void readCode(MethodVisitor mv, Context context, int u) {
        ...
        if ((context.flags & 2) == 0 && varTable != 0) {
            ...
            for (int i8 = this.readUnsignedShort(varTable); i8 > 0; --i8) {
                ...
                mv.visitLocalVariable(this.readUTF8(u + 4, c), this.readUTF8(u + 6, c), vsignature, labels[start], labels[start + length], index2);
                u += 10;
            }
        }
        if (tanns != null) {
