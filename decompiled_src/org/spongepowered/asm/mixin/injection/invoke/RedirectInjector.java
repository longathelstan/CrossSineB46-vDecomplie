...
extends InvokeInjector {
    ...
    protected void validateParams(RedirectedInvoke invoke) {
        ...
        for (int index2 = 0; index2 < argc; ++index2) {
            Type toType = null;
            if (index2 >= this.methodArgs.length) {
            ...
            Type fromType = this.methodArgs[index2];
            if (index2 < invoke.locals.length) {
                toType = invoke.locals[index2];
            } else {
                invoke.captureTargetArgs = true;
                argc = Math.max(argc, invoke.locals.length + invoke.target.arguments.length);
                int arg = index2 - invoke.locals.length;
                if (arg >= invoke.target.arguments.length) {
                    throw new InvalidInjectionException(this.info, String.format("%s has an invalid signature. Found unexpected additional target argument with type %s at index %d", description, fromType, index2));
                }
                toType = invoke.target.arguments[arg];
            }
            AnnotationNode coerce = Annotations.getInvisibleParameter(this.methodNode, Coerce.class, index2);
            if (fromType.equals(toType)) {
                if (coerce == null || !this.info.getContext().getOption(MixinEnvironment.Option.DEBUG_VERBOSE)) continue;
                Injector.logger.warn("Redundant @Coerce on {} argument {}, {} is identical to {}", new Object[]{description, index2, toType, fromType});
                continue;
            }
            boolean canCoerce = Injector.canCoerce(fromType, toType);
            if (coerce == null) {
                throw new InvalidInjectionException(this.info, String.format("%s has an invalid signature. Found unexpected argument type %s at index %d, expected %s", description, fromType, index2, toType));
            }
            if (canCoerce) continue;
            throw new InvalidInjectionException(this.info, String.format("%s has an invalid signature. Cannot @Coerce argument type %s at index %d to %s", description, toType, index2, fromType));
        }
    }
