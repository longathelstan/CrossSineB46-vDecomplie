...
implements IClassGenerator {
    ...
    private void generateIndexedSetter(String ref, String desc, Type[] args2, ClassVisitor writer) {
        ...
        for (index2 = 0; index2 < args2.length; index2 = (byte)(index2 + 1)) {
            set.visitVarInsn(21, 1);
            set.visitConstant((byte)index2);
            set.visitJumpInsn(159, labels[index2]);
        }
        ArgsClassGenerator.throwAIOOBE(set, 1);
        for (index2 = 0; index2 < args2.length; ++index2) {
            String boxingType = Bytecode.getBoxingType(args2[index2]);
            set.visitLabel(labels[index2]);
            set.visitVarInsn(21, 1);
            set.visitVarInsn(25, 2);
            set.visitTypeInsn(192, boxingType != null ? boxingType : args2[index2].getInternalName());
    ...
    private void generateMultiSetter(String ref, String desc, Type[] args2, ClassVisitor writer) {
        ...
        set.visitVarInsn(25, 0);
        set.visitFieldInsn(180, ref, VALUES_FIELD, OBJECT_ARRAY);
        for (byte index2 = 0; index2 < args2.length; index2 = (byte)(index2 + 1)) {
            set.visitInsn(89);
            set.visitConstant(index2);
            set.visitVarInsn(25, 1);
            set.visitConstant(index2);
            set.visitInsn(50);
            String boxingType = Bytecode.getBoxingType(args2[index2]);
