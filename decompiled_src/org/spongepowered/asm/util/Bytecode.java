...
public final class Bytecode {
    ...
    public static void loadArgs(Type[] args2, InsnList insns, int start, int end, Type[] casts) {
        ...
        for (Type type : args2) {
            ...
            if (casts != null && index2 < casts.length && casts[index2] != null) {
                insns.add(new TypeInsnNode(192, casts[index2].getInternalName()));
            }
    ...
    public static Object getConstant(AbstractInsnNode insn) {
        ...
        int index2 = Ints.indexOf((int[])CONSTANTS_ALL, (int)insn.getOpcode());
        return index2 < 0 ? null : CONSTANTS_VALUES[index2];
    }
    ...
    public static Type getConstantType(AbstractInsnNode insn) {
        ...
        int index2 = Ints.indexOf((int[])CONSTANTS_ALL, (int)insn.getOpcode());
        return index2 < 0 ? null : Type.getType(CONSTANTS_TYPES[index2]);
    }
    ...
    public static void compareBridgeMethods(MethodNode a, MethodNode b) {
            ...
            if (!(na instanceof LabelNode)) {
                if (na instanceof MethodInsnNode) {
                    ...
                    if (!ma.name.equals(mb.name)) {
                        throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_INVOKE_NAME, a.name, a.desc, index2, na, nb);
                    }
                    if (!ma.desc.equals(mb.desc)) {
                        throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_INVOKE_DESC, a.name, a.desc, index2, na, nb);
                    }
                } else {
                    if (na.getOpcode() != nb.getOpcode()) {
                        throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_INSN, a.name, a.desc, index2, na, nb);
                    }
                    if (na instanceof VarInsnNode) {
                        VarInsnNode va = (VarInsnNode)na;
                        VarInsnNode vb = (VarInsnNode)nb;
                        if (va.var != vb.var) {
                            throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_LOAD, a.name, a.desc, index2, na, nb);
                        }
                        ...
                        if (ta.getOpcode() == 192 && !ta.desc.equals(tb.desc)) {
                            throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_CAST, a.name, a.desc, index2, na, nb);
                        }
                    }
                }
            }
        ...
        if (ia.hasNext() || ib.hasNext()) {
            throw new SyntheticBridgeException(SyntheticBridgeException.Problem.BAD_LENGTH, a.name, a.desc, index2, null, null);
        }
    }
