...
extends MixinException {
    ...
    public SyntheticBridgeException(Problem problem, String name, String desc, int index2, AbstractInsnNode a, AbstractInsnNode b) {
        super(problem.getMessage(name, desc, index2, a, b));
    ...
    private PrettyPrinter printMethod(PrettyPrinter printer, MethodNode method) {
        ...
        while (iter.hasNext()) {
            printer.kv(index2 == this.index ? ">>>>" : "", Bytecode.describeNode((AbstractInsnNode)iter.next()));
            ++index2;
        }
        return printer.add();
    }
            ...
            case BAD_LOAD: {
                ...
                while (ia.hasNext() && ib.hasNext()) {
                    ...
                    if (na instanceof VarInsnNode && nb instanceof VarInsnNode) {
                        ...
                        printer.kv("Target " + index2, "%8s %-2d %s", Bytecode.getOpcodeName(va), va.var, ta);
                        printer.kv("Incoming " + index2, "%8s %-2d %s", Bytecode.getOpcodeName(vb), vb.var, tb);
                        if (ta.equals(tb)) {
    ...
    private PrettyPrinter printTypeComparison(PrettyPrinter printer, String index2, Type tpa, Type tpb) {
        printer.kv("Target " + index2, "%s", tpa);
        printer.kv("Incoming " + index2, "%s", tpb);
        if (tpa.equals(tpb)) {
    ...
    public static enum Problem {
        ...
        String getMessage(String name, String desc, int index2, AbstractInsnNode a, AbstractInsnNode b) {
            return String.format(this.message, name, desc, index2, Bytecode.getOpcodeName(a), Bytecode.getOpcodeName(a), Problem.getInsnName(a), Problem.getInsnName(b), Problem.getInsnDesc(a), Problem.getInsnDesc(b));
        }
