...
implements Serializable {
    ...
    @Nullable
    public final MatchResult matchAt(@NotNull CharSequence input, int index2) {
        MatcherMatchResult matcherMatchResult;
        Matcher matcher;
        Intrinsics.checkNotNullParameter(input, "input");
        Matcher $this$matchAt_u24lambda_u2d1 = matcher = this.nativePattern.matcher(input).useAnchoringBounds(false).useTransparentBounds(true).region(index2, input.length());
        boolean bl = false;
    ...
    @SinceKotlin(version="1.5")
    @ExperimentalStdlibApi
    public final boolean matchesAt(@NotNull CharSequence input, int index2) {
        Intrinsics.checkNotNullParameter(input, "input");
        return this.nativePattern.matcher(input).useAnchoringBounds(false).useTransparentBounds(true).region(index2, input.length()).lookingAt();
    }
