...
extends AbstractMutableList<E> {
    ...
    private final int positiveMod(int index2) {
        return index2 >= this.elementData.length ? index2 - this.elementData.length : index2;
    }

    private final int negativeMod(int index2) {
        return index2 < 0 ? index2 + this.elementData.length : index2;
    }

    @InlineOnly
    private final int internalIndex(int index2) {
        return this.positiveMod(this.head + index2);
    }

    private final int incremented(int index2) {
        return index2 == ArraysKt.getLastIndex(this.elementData) ? 0 : index2 + 1;
    }

    private final int decremented(int index2) {
        return index2 == 0 ? ArraysKt.getLastIndex(this.elementData) : index2 - 1;
    }
    ...
    public void add(int index2, E element) {
        AbstractList.Companion.checkPositionIndex$kotlin_stdlib(index2, this.size());
        if (index2 == this.size()) {
            this.addLast(element);
            return;
        }
        ...
        this.ensureCapacity(this.size() + 1);
        ArrayDeque arrayDeque = this;
        int internalIndex = arrayDeque.positiveMod(arrayDeque.head + index2);
        if (index2 < this.size() + 1 >> 1) {
    ...
    private final void copyCollectionElements(int internalIndex, Collection<? extends E> elements) {
        ...
        while (n < n2) {
            ...
            this.elementData[index2] = iterator2.next();
        }
        n = 0;
        n2 = this.head;
        while (n < n2) {
    ...
    public boolean addAll(int index2, @NotNull Collection<? extends E> elements) {
        ...
        ArrayDeque arrayDeque2 = this;
        int internalIndex = arrayDeque2.positiveMod(arrayDeque2.head + index2);
    ...
    public E get(int index2) {
        AbstractList.Companion.checkElementIndex$kotlin_stdlib(index2, this.size());
        ArrayDeque arrayDeque = this;
        ArrayDeque arrayDeque2 = this;
        int n = arrayDeque2.positiveMod(arrayDeque2.head + index2);
        return (E)arrayDeque.elementData[n];
    }
    ...
    public int indexOf(Object element) {
        block4: {
            ...
            block3: {
                ...
                int n2 = this.head;
                while (n2 < tail) {
                    if (!Intrinsics.areEqual(element, this.elementData[index2 = n2++])) continue;
            ...
            int n = this.head;
            index2 = this.elementData.length;
            while (n < index2) {
                int index3;
                if (!Intrinsics.areEqual(element, this.elementData[index3 = n++])) continue;
    ...
    public int lastIndexOf(Object element) {
        ...
        if (this.head < tail) {
            ...
            if (0 <= n3) {
                do {
                    int index3;
                    if (!Intrinsics.areEqual(element, this.elementData[index3 = n3--])) continue;
            ...
            if ((n = this.head) <= (n3 = ArraysKt.getLastIndex(this.elementData))) {
                int index4;
                do {
                    if (!Intrinsics.areEqual(element, this.elementData[index4 = n3--])) continue;
    ...
    public E removeAt(int index2) {
        AbstractList.Companion.checkElementIndex$kotlin_stdlib(index2, this.size());
        if (index2 == CollectionsKt.getLastIndex(this)) {
    ...
    private final boolean filterInPlace(Function1<? super E, Boolean> predicate) {
        ...
        if (this.head < tail) {
            ...
            int n4 = this.head;
            int index2 = this.elementData.length;
            while (n4 < index2) {
                int index3 = n4++;
                Object element = this.elementData[index3];
                ((ArrayDeque)this).elementData[index3] = null;
                if (predicate.invoke(element).booleanValue()) {
                    int n5 = newTail;
                    newTail = n5 + 1;
