...
extends Module {
    ...
    private final IntegerValue red = (IntegerValue)new IntegerValue("Red", 255, 0, 255).displayable(() -> (Boolean)this.clientTheme.get() == false);
    private final IntegerValue green = (IntegerValue)new IntegerValue("Green", 255, 0, 255).displayable(() -> (Boolean)this.clientTheme.get() == false);
    private final IntegerValue blue = (IntegerValue)new IntegerValue("Blue", 255, 0, 255).displayable(() -> (Boolean)this.clientTheme.get() == false);
    ...
    public void onRender(Render3DEvent event) {
        ...
        for (int i = 0; i < playersLength; ++i) {
            ...
            if (entPos == null || player.func_145782_y() == -1488 || !player.func_70089_S() || !RenderUtils.isInViewFrustrum((Entity)player) || player.field_70128_L || player == Skeletal.mc.field_71439_g || !EntityUtils.INSTANCE.isSelected((Entity)player, true) || player.func_70608_bn() || player.func_82150_aj()) continue;
            GL11.glPushMatrix();
            float[][] modelRotations = (float[][])playerRotationMap.get(player);
            GL11.glLineWidth((float)1.0f);
            if (((Boolean)this.clientTheme.get()).booleanValue()) {
