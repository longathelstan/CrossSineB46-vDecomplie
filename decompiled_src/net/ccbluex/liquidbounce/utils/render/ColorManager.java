...
public class ColorManager {
    ...
    public static int rainbow(int delay2) {
        double rainbowState = Math.ceil((double)(System.currentTimeMillis() + (long)delay2) / 20.0);
    ...
    public static int fluxRainbow(int delay2, long timeOffset, float sa) {
        double rainbowState = Math.ceil((double)((System.currentTimeMillis() + timeOffset) / 8L) + (double)delay2 / 20.0);
    ...
    public static int getRainbow3(int tick) {
        double d = 0.0;
        double delay2 = Math.ceil((System.currentTimeMillis() + (long)(tick * 2)) / 5L);
        float rainbow = (double)((float)(d / 360.0)) < 0.5 ? -((float)(delay2 / 360.0)) : (float)((delay2 %= 360.0) / 360.0);
        return Color.getHSBColor(rainbow, 0.5f, 1.0f).getRGB();
    }

    public static int astolfoRainbow(int delay2, int offset, int index2) {
        double d;
