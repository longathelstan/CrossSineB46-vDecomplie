...
public class TTFFontRenderer {
    ...
    private CharacterData[] setup(CharacterData[] characterData, int type) {
        ...
        for (int index2 = 0; index2 < characterData.length; ++index2) {
            char character = (char)index2;
            ...
            characterData[index2] = new CharacterData(character, characterImage.getWidth(), characterImage.getHeight(), textureId);
        }
    ...
    private void renderString(String text, float x, float y, int color, boolean shadow) {
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (character == '\u00a7' && i < length) {
                int index2 = "0123456789abcdefklmnor".indexOf(text.toLowerCase(Locale.ENGLISH).charAt(i + 1));
                if (index2 < 16) {
                    obfuscated = false;
                    strikethrough = false;
                    underlined = false;
                    ...
                    int textColor = this.colorCodes[index2];
    ...
    private void generateColors() {
        for (int i = 0; i < 32; ++i) {
            int thingy = (i >> 3 & 1) * 85;
            int red2 = (i >> 2 & 1) * 170 + thingy;
            int green2 = (i >> 1 & 1) * 170 + thingy;
            int blue2 = (i >> 0 & 1) * 170 + thingy;
            if (i == 6) {
                red2 += 85;
            }
            if (i >= 16) {
            ...
            this.colorCodes[i] = (red2 & 0xFF) << 16 | (green2 & 0xFF) << 8 | blue2 & 0xFF;
        }
    }
